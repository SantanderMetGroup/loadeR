...
#' 
#' Makes a logical subset of a seasonal forecast GeoGrid using the parameters specified by the user,
#' applying the java methods makeSubset and readDataSlice. Subroutine of loadSeasonalForecast.
#' 
...
#' @param memberRangeList A list of ensemble java ranges as returned by getMemberDomain
#' @param foreTimePars A list of forecast time definition parameters, as returned by getForecastTimeDomain.S4
#' @param verticalPars A list with vertical level definition, as returned by \code{getVerticalLevelPars}.
#'  Only the last element (the java index position) is used
...
makeSubset.seasonal <- function(grid, latLon, runTimePars, memberRangeList, foreTimePars, verticalPars) {
