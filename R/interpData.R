...
#' The bilinear interpolator is a wrapper of function \code{akima::interp} 
#' in package \pkg{akima}.
...
interpData <- function(obj, new.Coordinates = list(x = NULL, y = NULL), method = c("nearest", "bilinear")) {
      ...
      if (any(attr(obj$Data, "dimensions") == "station")){
            ...
            if (is.vector(x) & is.vector(y)){
                  x <- t(list(x=outer(obj$xyCoords$y*0,obj$xyCoords$x,FUN="+"), y=outer(obj$xyCoords$y,obj$xyCoords$x*0,FUN="+"))$x)
                  y <- t(list(x=outer(obj$xyCoords$y*0,obj$xyCoords$x,FUN="+"), y=outer(obj$xyCoords$y,obj$xyCoords$x*0,FUN="+"))$y)
            }
      }
      ...
      if (is.null(n.members)) {
            ...
            interp.list <- lapply(1:dim(obj$Data)[time.ind], function(j) {
                  ...
                  if (method == "bilinear" & length(indNoNA) != 0) {
                        int <- interp(x = x[indNoNA], y=y[indNoNA], z[indNoNA], xo = new.Coordinates$x, yo = new.Coordinates$y,
                                      linear = TRUE, extrap=FALSE, duplicate = "error", dupfun = NULL,ncp = NULL, nx = length(new.Coordinates$x), ny = length(new.Coordinates$y))
                        if (any(attr(new.Coordinates,"type") == "location")) {
            ...
            if (any(attr(new.Coordinates,"type") == "location")) {
                  ...
                  attr(gridData$Data, "dimensions") <- c("station", "time", "member")
            }else{
