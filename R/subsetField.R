...
subsetVar <- function(multiField, var) {
      ...
      attributes(multiField$Variable)[-1] <- lapply(attributes(multiField$Variable)[-1], "[", var.idx)
...
subsetDimension <- function(field, dimension = NULL, indices = NULL) {
      ...
      if (!is.null(indices)){
            ...
            warning("Argument 'indices' is NULL and no subsetting has been applied. The same input 'field' is returned.")
      }
      return(field)
}
# End
